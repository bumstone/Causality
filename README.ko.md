...
Ouroboros HITL은 Claude, Codex, Ouroboros 계열 프로젝트를 위한 로컬 우선·의존성
...
flowchart TD
    ...
    R -->|장기 작업| P3["컨트랙트 하네스 + limited Ouroboros loop"]
...
CLAUDE.md                                  # Claude 지침
.codex/ouroboros-routing.md                # Codex 라우팅 컨텍스트
...
.ouroboros/agent-rules.md                  # 얇은 규칙 + 라우팅
.ouroboros/ouroboros-workflows.json        # 워크플로 manifest
.ouroboros/mcp.json                        # MCP 서버 설정
.ouroboros/ledger.jsonl                    # append-only 증거 원장
...
```bash
ouroboros-hitl init            # 원장과 워크플로 manifest 생성
ouroboros-hitl manifest        # 워크플로 manifest 출력 (--pretty 추가)
ouroboros-hitl context         # 원장 tail + 워크플로 이름 출력 (--pretty, --limit N)
ouroboros-hitl install-agent   # 프로젝트 수준 에이전트 자동화 설치 (--project, --force)
```
...
Codex는 `AGENTS.md`, `.codex/ouroboros-routing.md`, `.ouroboros/agent-rules.md`를
자동 라우팅 컨텍스트로 사용합니다.

- 계획 / 스펙 요청 -> `ouroboros-plan`
- 구현 또는 검증 요청 -> `ouroboros-verify`
- 버그 / 회귀 요청 -> `ouroboros-root-cause`
- 브라우저 / UI 흐름 요청 -> `ouroboros-a11y-observe`
- "완료" / "배포" / 최종 핸드오프 -> `ouroboros-complete`
...
생성된 `.ouroboros/mcp.json`에 동일한 커맨드가 들어 있습니다. 노출 도구:

- `ouroboros_init` — 프로젝트 수준 에이전트 자동화 파일 설치
- `ouroboros_context` — 원장 tail과 워크플로 이름 반환
- `ouroboros_append_evidence` — `.ouroboros/ledger.jsonl`에 증거 추가
- `ouroboros_workflows` — 워크플로 manifest 반환
...
- `orchestrator.py` — 원장과 게이트를 감싸는 `OuroborosHITL` 퍼사드.
