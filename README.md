...
| [0002](docs/adr/0002-three-layer-control-stack.md) | Three-layer control stack | Proposed |
| [0003](docs/adr/0003-contract-harness.md) | Contract Harness | **Accepted / Implemented** |
| [0004](docs/adr/0004-agent-harness-task-routing.md) | Agent Harness task routing | Proposed |
| [0005](docs/adr/0005-identity-memory-skill-substrate.md) | Identity / memory / skill substrate | Proposed |
| [0006](docs/adr/0006-final-blended-architecture.md) | Final blended (5-layer) architecture | Proposed |
| [0007](docs/adr/0007-context-economy-progressive-disclosure.md) | Context Economy / progressive disclosure | **Accepted / Partial** |

Implemented today: the `non_goals` field, the frozen `TaskContract`, the
enforcing gates (`check_tool_allowed` / `check_non_goal` / `should_stop`),
`ContractHarness.bind`, and the on-demand file layout
(`workflow/` `checklists/` `skills/` `memory/<6 types>/`) with the Context
Economy operating rule. Not yet implemented: the loop driver that calls
`should_stop` each iteration, Review automation, Reflect / Skill update, the
Agent Harness dispatcher, and three-layer execution metadata.
