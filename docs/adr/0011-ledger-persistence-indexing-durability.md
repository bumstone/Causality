...
- **상태(Status):** Accepted — 부분 구현 (R2 캐시 구현, R4 durable-write 결정·미구현)
- **날짜:** 2026-06-13
...
- `latest_hash`만 캐시하고 **파일 크기로 무효화**. append 전체 스캔을 없애 단일 writer
  append를 **amortized O(1)**로. 같은 파일의 다른 인스턴스가 append하면(mcp_server 상주
  ledger vs install_agent_files) 크기 변화가 캐시를 무효화해 tail을 재읽는다 → stale·체인
  깨짐 방지(codex r3407872680). append-only라 크기 단조 증가.
- `events()`/`find()`는 호출마다 디스크에서 새로 파싱 → 공유 가변 캐시 없음. 반환 이벤트
  payload를 mutate해도 원장 오염 없음(codex r3407872681).
- `events_for_contract`/`latest_hash_for_contract` 접근자가 손필터를 대체해 footgun을 구조
  제거(reflect·skills 사용).

### 2.2 R4 — durable write path (결정, 미구현)

ledger/memory/skills/agenda 쓰기를 공통 헬퍼로 통일: **R4a** read/append 중복 제거,
**R4b** temp write + `os.replace` + `fsync` + torn-line 복구, **R4c** `flock`(cross-process
직렬화). 전체 read-path 인덱싱도 R4에서 다룬다.
...
- **긍정:** append O(N²)→amortized O(1), footgun 구조 제거, 같은 파일 형제 인스턴스 정확성
  유지. JSONL·해시 체인·기존 시그니처 불변(접근자만 추가).
- **비용:** `events()` 계열은 여전히 호출마다 전체 read(기존과 동일, 무회귀). 전체 인덱싱은
  R4 후속.
