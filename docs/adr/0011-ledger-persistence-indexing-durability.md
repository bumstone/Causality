...
- **상태(Status):** Accepted — 구현 (R2 캐시 + R4 durable write path; 전체 read-path 인덱싱만 후속)
- **날짜:** 2026-06-13
...
### 2.2 R4 — durable write path (구현, `durable.py`)

4개 저장소 쓰기를 공통 헬퍼로 통일. **R4a:** read/append/rewrite를 `DurableJsonl`+
`write_text_durably`로 추출. **R4b:** rewrite=temp+`fsync`+`os.replace`(원자), append=
`fsync`+torn tail 절단, read=torn 마지막 줄 drop. **R4c:** `<path>.lock` `flock`로 writer
직렬화; ledger는 read-latest+append를 락 내 수행해 체인 fork 방지.
...
- **중립:** size 신호는 append-only 전제. cross-process 직렬화는 POSIX `flock`(R4c) 한정.
